...
export class StoreClientService {
    ...
    public async execute({ cpf, logradouro, nome, bairro, cep, complemento, email, telefone, numero, cidade, cod, estado
    }: Props) {
        ...
        if (cod) {
            ...
            const [insert] = await this.conn('coalemos.clientes').insert(payload).returning('cod');
